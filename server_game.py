...
def handle_client(conn, addr):
    ...
    connected = True
    while connected:
        msg_length = conn.recv(HEADER).decode(FORMAT) #How long the message is
        if msg_length: #Checking whether received message has something
            msg_length = int(msg_length)
            msg = conn.recv(msg_length).decode(FORMAT) #How many bytes we are receiving and decoding
...
def start(): #Starting socket server allowing server to listen then passing them to handle_client
    ...
    while True:
        ...
        thread.start()
        #To start the game until new connection is established
